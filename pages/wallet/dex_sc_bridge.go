...
var _ router.Page = &PageDEXSCBridge{}

func NewPageDEXSCBridge() *PageDEXSCBridge {
...
func (p *PageDEXSCBridge) IsActive() bool {
	return p.isActive
}

func (p *PageDEXSCBridge) Enter() {
	p.isActive = true

	if !page_instance.header.IsHistory(PAGE_DEX_SC_BRIDGE) {
...
func (p *PageDEXSCBridge) Leave() {
	p.animationLeave.Start()
	p.animationEnter.Reset()
}

func (p *PageDEXSCBridge) SetToken(token *wallet_manager.Token) {
	p.token = token
...
func (p *PageDEXSCBridge) submitForm() error {
...
func (p *PageDEXSCBridge) Layout(gtx layout.Context, th *material.Theme) layout.Dimensions {
	{
	...
	widgets = append(widgets, func(gtx layout.Context) layout.Dimensions {
		return layout.Flex{Axis: layout.Vertical}.Layout(gtx,
			...
			layout.Rigid(func(gtx layout.Context) layout.Dimensions {
				...
				return p.infoRows[1].Layout(gtx, th, lang.Translate("Bridge Fee"), fmt.Sprintf("%s DERO", amount.Format()))
			}),
		)
	})
